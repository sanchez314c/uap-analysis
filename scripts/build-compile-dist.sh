...
# Function to build Python applications
build_python_app() {
    ...
    if [[ "$major_minor" < "3.11" ]]; then
        print_warning "WARNING: Python $major_minor detected."
        print_warning "For best results on macOS, use Python 3.11 or later."

        # Check for Python 3.11
        if command -v python3.11 &> /dev/null; then
            print_status "Found Python 3.11, switching to that version..."
            PYTHON_CMD="python3.11"
        else
            print_error "ERROR: Python 3.11+ required for proper GUI rendering"
    ...
    # Check for spec file
    if [ -f "UAP Video Analyzer.spec" ]; then
        print_status "Building with PyInstaller using spec file..."
        pyinstaller "UAP Video Analyzer.spec" --noconfirm --clean
    else
        print_status "Building with PyInstaller..."
        pyinstaller --onefile --windowed --name="UAP Video Analyzer" \
            --add-data="src:src" \
            --add-data="configs:configs" \
            --add-data="data:data" \
            --add-data="assets:assets" \
            src/gui/stable_gui.py
    fi
    ...
    print_success "Python application built successfully"
}

# Function to run comprehensive build
run_comprehensive_build() {
    ...
    case "$DETECTED_STACK" in
        ...
        "electron")
            ...
            ;;
        *)
            print_warning "No build configuration for stack: $DETECTED_STACK"
            ;;
    esac
